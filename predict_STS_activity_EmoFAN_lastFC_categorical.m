...
video_imageFeatures = table2array(readtable('/home/data/eccolab/Code/GitHub/emonet/emonet_face_output_NNDB_lastFC.txt'));
...
for s = 5:length(subjects)
    ...
    [~,~,~,~,b] = plsregress(timematched_features,masked_dat.dat',8); % b = regression coefficient (beta)
    disp('beta done')
    ...
    for k=1:5
        ...
        [xl,yl,xs,ys,beta_cv,pctvar] = plsregress(timematched_features(kinds~=k,:), masked_dat.dat(:,kinds~=k)', min(8,size(masked_dat.dat,1))); %size(masked_dat.dat,1)=252
    ...
    save(['sub-' subjects{s} '_STS_emoFAN_lastFC_invert_imageFeatures_output_categorical.mat'], 'mean_diag_corr', '-v7.3')

    %UPDATED
    save(['beta_sub-' subjects{s} '_STS_emoFAN_lastFC_invert_imageFeatures_categorical.mat'], 'b', '-v7.3')
