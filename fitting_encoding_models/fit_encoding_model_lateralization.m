function fit_encoding_model_lateralization(model,layer,region,side)
...
% ::
%
%   fit_encoding_model(model,layer,region,side)
%
% ..
...
output_directory = '/home/data/eccolab/Code/NNDb/SEE/outputs/lateralization';
...
if strcmp(region,'Amy')
    if strcmp(side,'left')
        rcell = {'Amygdala_CM__L','Amygdala_SF__L','Amygdala_AStr__L','Amygdala_LB__L'};
    elseif strcmp(side,'right')
        rcell = {'Amygdala_CM__R','Amygdala_SF__R','Amygdala_AStr__L','Amygdala_LB__L'};
    end
elseif strcmp(region,'pSTS')
    if strcmp(side,'left')
        rcell = {'Ctx_STSvp_L','Ctx_STSdp_L'};
    elseif strcmp(side,'right')
        rcell = {'Ctx_STSvp_R','Ctx_STSdp_R'};
    end
...
elseif strcmp(region,'V1')
    ...
    error('Please specify region as either "Amy" , "pSTS", "FFA", "OFA", or "V1" and side as "left" or "right"')
end
...
for s = 1:length(subjects)
	...
	%excluded_voxels(s,:) = masked_dat.removed_voxels;
        atlas_amyg_split = split_atlas_by_hemisphere(select_atlas_subset(load_atlas('canlab2018'),'Amy'))
	masked_dat = apply_mask(dat,select_atlas_subset(atlas_amyg_split,rcell));
    else
	masked_dat = apply_mask(dat,select_atlas_subset(load_atlas('canlab2018'),rcell));
    end
    ...
    save([output_directory filesep 'sub-' subjects{s} '_' region '_' side '_' model '_' layer '_yhat.mat'], 'yhat', '-v7.3') %save predictors for each subject


    save([output_directory filesep 'sub-' subjects{s} '_' region '_' side '_' model '_' layer '_mean_diag_corr.mat'], 'mean_diag_corr', '-v7.3') %save performance measures for each subject

    save([output_directory filesep 'beta_sub-' subjects{s} '_' region '_' side '_'  model '_' layer '_mean_diag_corr.mat'], 'b', '-v7.3') %save betas for each subject
...
if strcmp(model,'combined') && strcmp(layer,'intermediate')
    save([output_directory filesep 'noise_ceiling' region side 'combined_intermed.mat'], 'mean_diag_corr')
elseif  strcmp(model,'emonet') && strcmp(layer,'intermediate')
    save([output_directory filesep 'noise_ceiling' region side 'emonet_intermed.mat'], 'mean_diag_corr')
elseif  strcmp(model,'emofan') && strcmp(layer,'intermediate')
    save([output_directory filesep 'noise_ceiling' region side 'emofan_intermed.mat'], 'mean_diag_corr')

elseif  strcmp(model,'combined') && strcmp(layer,'late')
    save([output_directory filesep 'noise_ceiling' region side 'combined_late.mat'], 'mean_diag_corr')
elseif  strcmp(model,'emonet') && strcmp(layer,'late')
    save([output_directory filesep 'noise_ceiling' region side 'emonet_late.mat'], 'mean_diag_corr')
elseif  strcmp(model,'emofan') && strcmp(layer,'late')
    save([output_directory filesep 'noise_ceiling' region side 'emofan_late.mat'], 'mean_diag_corr')
