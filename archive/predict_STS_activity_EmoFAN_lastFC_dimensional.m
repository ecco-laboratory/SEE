...
for s = 1:length(subjects)
    ...
    [~,~,~,~,b] = plsregress(timematched_features,masked_dat.dat',2); % b = regression coefficient (beta)
    disp('beta done')
    ...
    for k=1:5
        ...
        [xl,yl,xs,ys,beta_cv,pctvar] = plsregress(timematched_features(kinds~=k,:), masked_dat.dat(:,kinds~=k)', min(2,size(masked_dat.dat,1))); %size(masked_dat.dat,1)=252
    ...
    save(['sub-' subjects{s} '_STS_emoFAN_lastFC_invert_imageFeatures_output_dim.mat'], 'mean_diag_corr', '-v7.3')

    %UPDATED
    save(['beta_sub-' subjects{s} '_STS_emoFAN_lastFC_invert_imageFeatures_dim.mat'], 'b', '-v7.3')
