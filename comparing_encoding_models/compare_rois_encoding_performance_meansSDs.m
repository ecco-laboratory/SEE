...
%% means and SDs of the prediction-outcome correlation
STS_emoFAN_mean = mean(avgcorr_pSTS_EmoFAN_final);
STS_emoFAN_sd = std(avgcorr_pSTS_EmoFAN_final);

STS_emonet_mean = mean(avgcorr_pSTS_EmoNet);
STS_emonet_sd = std(avgcorr_pSTS_EmoNet);

amygdala_emoFAN_mean = mean(avgcorr_amygdala_EmoFAN_final);
amygdala_emoFAN_sd = std(avgcorr_amygdala_EmoFAN_final);

amygdala_emonet_mean = mean(avgcorr_amygdala_EmoNet);
amygdala_emonet_sd = std(avgcorr_amygdala_EmoNet);

%%
STS_combo_mean = mean(avgcorr_pSTS_multi);
STS_combo_sd = std(avgcorr_pSTS_multi);

amygdala_combo_mean = mean(avgcorr_amygdala_multi);
amygdala_combo_sd = std(avgcorr_amygdala_multi);

%%
STS_emoFAN_int_mean = mean(avgcorr_pSTS_EmoFAN_conv);
STS_emoFAN_int_sd = std(avgcorr_pSTS_EmoFAN_conv);

STS_emonet_int_mean = mean(avgcorr_pSTS_EmoNet_fc7);
STS_emonet_int_sd = std(avgcorr_pSTS_EmoNet_fc7);

amygdala_emoFAN_int_mean = mean(avgcorr_amygdala_EmoFAN_conv);
amygdala_emoFAN_int_sd = std(avgcorr_amygdala_EmoFAN_conv);

amygdala_emonet_int_mean = mean(avgcorr_amygdala_EmoNet_fc7);
amygdala_emonet_int_sd = std(avgcorr_amygdala_EmoNet_fc7);
